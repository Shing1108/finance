...
// 匯率數據
// 默認匯率數據
if (typeof exchangeRates === 'undefined' || !exchangeRates) {
    console.log("初始化默認匯率數據");
    exchangeRates = {
...
function getExchangeRate(fromCurrency, toCurrency) {
    console.log(`獲取匯率: ${fromCurrency} → ${toCurrency}`);
    ...
    try {
        ...
        // 檢查exchangeRates是否已初始化
        if (!exchangeRates) {
            console.error("匯率數據尚未初始化");
            return 1;
        }

        // 如果有直接匯率，則使用
        if (exchangeRates[fromCurrency] && 
            typeof exchangeRates[fromCurrency][toCurrency] !== 'undefined') {
            return exchangeRates[fromCurrency][toCurrency];
        }

        // 如果有反向匯率，則使用其倒數
        if (exchangeRates[toCurrency] && 
            typeof exchangeRates[toCurrency][fromCurrency] !== 'undefined') {
        ...
        // 如果還是無法轉換，則返回1
        console.warn(`無法找到匯率: ${fromCurrency} 至 ${toCurrency}`);
        return 1;
    } catch (error) {
...
function createCurrencyManagementModal() {
    ...
    if (saveManualRateButton) {
        saveManualRateButton.addEventListener('click', function() {
            if (typeof saveManualExchangeRate === 'function') {
                saveManualExchangeRate();
            } else {
                console.error('saveManualExchangeRate 函數未定義');
                showToast('保存匯率失敗: 找不到保存函數', 'error');
            }
        });
    }
    ...
    console.log("貨幣管理模態框創建完成");
}

// 更新匯率表格
function updateExchangeRateTable() {
    console.log("開始更新匯率表格");
    
    const table = document.getElementById('exchangeRateTable');
    if (!table) {
        console.error('找不到匯率表格元素 (id: exchangeRateTable)');
        return;
    }
    ...
    // 檢查匯率數據是否可用
    console.log("當前匯率數據:", exchangeRates);
        ...
        currencies.forEach(toCurrency => {
            ...
            if (fromCurrency === toCurrency) {
                ...
                try {
                    ...
                    console.error(`計算匯率出錯 (${fromCurrency} → ${toCurrency}):`, e);
    ...
    if (lastRateUpdateTime) {
        ...
        if (storedRates) {
            try {
                ...
                lastRateUpdateTime.textContent = "上次更新: 格式錯誤";
            }
        } else {
            lastRateUpdateTime.textContent = "上次更新: 無記錄";
        }
    } else {
        console.warn("找不到lastRateUpdateTime元素");
    }
...
function previewExchangeRate() {
    try {
        ...
        console.log(`預覽匯率: ${fromCurrency} → ${toCurrency}`);
    ...
    try {
        ...
        setTimeout(() => {
            if (typeof updateExchangeRateTable === 'function') {
                console.log("開始填充匯率表格");
                updateExchangeRateTable();
            } else {
                console.error("updateExchangeRateTable 函數未定義");
            }
        }, 100);
        ...
        if (typeof openModal === 'function') {
            ...
            console.log("貨幣管理模態框已打開");
        } else {
            console.error("openModal 函數未定義");
            // 備用方案

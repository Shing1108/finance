    ...
    try {
        ...
        switch (tabId) {
            ...
            case 'transactions':
                ...
                break;
            // ui.js 中的 showTabContent 函數內
            case 'budgets':
                if (typeof updateBudgetsUI === 'function') {
                    updateBudgetsUI();
                    } else {
                    console.error("找不到 updateBudgetsUI 函數");
                    showToast('更新預算UI失敗: 找不到更新函數', 'error');
                }
                break;
            case 'categories':
                // 確保類別管理標籤的初始化
...
function initializeBudgetsTab(budgetsTab) {
    ...
    document.getElementById('autoCalculateBudget').addEventListener('change', function() {
...
function updateBudgetProgress() {
    ...
    try {
        ...
        if (!budgetProgressContainer) {
            console.warn("找不到預算進度容器，預算進度部分可能不會顯示");
            return;
        }
...
function updateCategoryBudgets() {
    ...
    try {
        ...
        if (!categoryBudgetsList) {
            console.warn("找不到類別預算列表，類別預算部分可能不會顯示");
            return;
        }
...
function updateCategoryBudgetSelect() {
    ...
    try {
        ...
        // 清空選擇器
        select.innerHTML = '<option value="" disabled selected>選擇類別</option>';
...
function checkBudgetAlerts() {
    ...
    try {
        ...
        if (percentage >= alertThreshold) {
            showToast(`預算警告: 已使用 ${percentage.toFixed(1)}% 的預算`, 'warning');
        }
    } catch (error) {
...
// 安全地綁定事件監聽器
function safelyBindEventListener(elementId, eventType, callback) {
    const element = document.getElementById(elementId);
    if (element) {
        element.addEventListener(eventType, callback);
        return true;
    } else {
        console.warn(`找不到元素 ${elementId} 來綁定 ${eventType} 事件`);
        return false;
    }
}
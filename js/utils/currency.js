        ...
        try {
            ...
            if (savedRates) {
                ...
                if (parsed.rates) {
                    ...
                    lastRateUpdate = parsed.lastUpdated;
                    console.log("已從本地載入匯率數據");
                }
            }
            
            // 如果超過24小時沒有更新匯率，嘗試自動更新
            const now = new Date();
            const lastUpdate = lastRateUpdate ? new Date(lastRateUpdate) : null;
            
            if (!lastUpdate || (now - lastUpdate) > 24 * 60 * 60 * 1000) {
                console.log("匯率超過24小時未更新，嘗試自動更新");
                // 使用延遲載入不阻塞UI
                setTimeout(() => {
                    updateExchangeRatesFromAPI().catch(err => 
                        console.warn("自動更新匯率失敗:", err)
                    );
                }, 2000);
            }
        } catch (error) {
...
// 為了與舊代碼兼容，將關鍵函數導出到全局
window.getExchangeRate = window.currencyManager.getExchangeRate;
window.convertCurrency = window.currencyManager.convertCurrency;
window.updateExchangeRatesFromAPI = window.currencyManager.updateExchangeRatesFromAPI;
window.showExchangeRatesTable = window.currencyManager.showExchangeRatesTable;
window.initExchangeRates = window.currencyManager.initExchangeRates;
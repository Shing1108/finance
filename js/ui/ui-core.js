        ...
        switch (tabId) {
            ...
            case 'accounts':
                ...
                case 'transactions':
                    ...
                    if (typeof initializeTransactionTabs === 'function') {
 ...
 * @param {number} duration 顯示時間(毫秒)，默認5000ms
 */
function showToast(message, type = 'info', duration = 5000) {
    ...
    try {
        ...
        if (closeButton) {
            closeButton.addEventListener('click', () => fadeOutToast(toast, toastContainer));
        }
...
function updateAllUI() {
    ...
    // 使用Promise來處理可能的非同步更新
    const updatePromises = [];

    try {
        // 優先更新基本UI元素
        updatePromises.push(updateBasicUI());
        
        // 更新其他UI組件
        updatePromises.push(updateComponentUI());
        ...
        Promise.all(updatePromises)
            ...
            .catch(error => {
                console.error("UI更新過程中發生錯誤:", error);
                showToast('部分UI更新失敗，請嘗試重新載入頁面', 'warning');
...
function updateBasicUI() {
    return new Promise((resolve, reject) => {
        try {
            ...
            console.error("更新基本UI元素時發生錯誤:", error);
            reject(error);
        }
    });
}
...
function updateComponentUI() {
    return new Promise((resolve, reject) => {
        try {
            // 使用setTimeout將UI更新放入下一個事件循環，減少阻塞
            setTimeout(() => {
                try {
                    // 更新賬戶管理
                    if (typeof updateAccountsUI === 'function') {
                    ...
                    console.error("更新組件UI時發生錯誤:", error);
                    reject(error);
                }
            }, 0);
        } catch (error) {
            console.error("設置組件UI更新時發生錯誤:", error);
            reject(error);
        }
    });
}
